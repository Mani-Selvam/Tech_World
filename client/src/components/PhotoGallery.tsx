    ...
    useEffect(() => {
        ...
        const updateScrollSpeed = () => {
            if (window.innerWidth < 640) {
                setScrollSpeed(7); // Fast on mobile
                        ...
                        {duplicatedImages.map((image, index) => (
                            <div key={index} className="flex-shrink-0">
                                <img
                                    src={image.src}
                                    alt={image.alt}
                                    className="w-60 sm:w-72 md:w-80 h-48 sm:h-56 md:h-60 object-cover rounded-lg hover:scale-105 transition-transform duration-1000"
